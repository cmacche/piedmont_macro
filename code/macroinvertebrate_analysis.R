...
df_table = df_invert %>% 
  ...
  summarize(n_na_family = sum(is.na(Family)) + sum(Family == "Unknown", na.rm = T),
            ...
            p_na_latin = round(n_na_latin / n * 100, 2))
df_table = filter(df_table, p_na_family <= 10) %>% 
  filter(df_table, p_na_family <= 10)
...
df_invertwq = merge(df_invert, df_wq, by = c("Date", 
                                        "Latitude","Longitude"))
df_invertwq2 = merge(df_invert2, df_wq, by = c("Date", 
                                              "Latitude","Longitude"))

df_invertwq$Date = as.Date(df_invertwq$Date , format = "%m/%d/%y")

species = unique(df_invertwq$Class)
fit = list(NULL)

for (i in 1:length(species)) {
  fit[[i]] =glm.nb(Abundance ~ pH_SU + Sp_Cond + Diss_Oxy + Temp_C , 
                    data = df_invertwq %>% filter(Class == species[i]))
}
...
pre_invertwq = subset(pre_invertwq, 
                      select = c("Abundance", "pH_SU", "Sp_Cond","Temp_C","Diss_Oxy"))
invertwq_glm = glm.nb(Abundance~ ., data = df_invertwq)
